...
def test_execute_valid_file(tmp_path):
    ...
    inc = Include(type="test", source=str(file))
...
def test_execute_valid_symlink(tmp_path):
    ...
    inc = Include(type="test", source=str(link))
    assert inc.execute("", {}) == "linked"

def test_execute_missing_file():
    inc = Include(type="test", source="/nonexistent/path.txt")
    with pytest.raises(FileNotFoundError):
        inc.execute("", {})

def test_execute_directory(tmp_path):
    inc = Include(type="test", source=str(tmp_path))
    with pytest.raises(FileNotFoundError):
        inc.execute("", {})